...
# This provides a comprehensive overview of cell abundances and helps identify which cell types are most prevalent across different tissues.
...
# %%
# The bar chart shows that macrophages represent the largest proportion (~34%) of lung cells, followed by AT2 cells (~25%) and monocytes (~7%).
# In contrast, rare cell types such as NK cells, lymphatic endothelial cells, and ionocytes account for less than 2%.
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# To further characterize a specific cell type within a tissue, marker genes can be retrieved using the ``markers`` method.
...
# %%
# To analyze how these marker genes are expressed across different lung cell types, use the ``average`` method:
...
#
# Marker gene expression is generally higher in macrophages than in other lung cell types.
# For example, *MARCO* shows an average expression of 12.84 cptt in macrophages, compared to just 0.12 in neutrophils and 0.18 in B cells.
# This confirms *MARCO* as a highly specific marker for macrophages in the lung.

# %%
# Visualizing marker gene expression with a heatmap
# -------------------------------------------------
# While the table provides a detailed quantitative view, visualizing the expression data as a heatmap can help highlight global patterns
# and make comparisons across cell types more intuitive.
...
# %%
# The heatmap displays the average expression of selected marker genes across lung cell types, with color intensity representing expression levels in counts per ten thousand (cptt).
# Darker colors indicate lower expression, while lighter shades (e.g., pink and white) reflect higher expression. Genes such as *MARCO* and *MSR1* exhibit strong, macrophage-specific expression, confirming their relevance as marker genes for this cell type.

# %%
# Conclusion
# ^^^^^^^^^^
# This tutorial introduced how to explore cell types, their distribution, and marker genes using a single species' atlas.
# The human lung served as an example, but the same approach can be applied to any other species or cell type available in
# the approximated cell atlases. Try adjusting the cell_type and organism parameters to explore new biological insights.
#
